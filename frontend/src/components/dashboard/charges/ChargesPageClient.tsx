...
import { ClipboardList, Eye, Pencil, Plus, Search, Trash2, X } from 'lucide-react'
import { api } from '@/api'
...
export default function ChargesPageClient() {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-3"><div className="rounded-md border bg-white p-4"><p className="text-xs text-slate-500">Total filtré</p><p className="mt-1 text-2xl font-bold">{total.toLocaleString('fr-FR')} FCFA</p></div><div className="rounded-md border bg-white p-4"><p className="text-xs text-slate-500">Nombre de charges</p><p className="mt-1 text-2xl font-bold">{charges.length}</p></div><div className="rounded-md border bg-white p-4"><p className="text-xs text-slate-500">Catégories représentées</p><p className="mt-1 text-2xl font-bold">{new Set(charges.map(c => c.category)).size}</p></div></div>
    <div className="rounded-md border bg-white">
      ...
      <div className="flex justify-end border-b p-3"><ExportButtons onExportExcel={() => exportToCsv('charges', rows)} onExportPdf={() => exportToPrint('Rapport des charges — SPGCR')} /></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="p-4">Date</th><th>Libellé</th><th>Catégorie</th><th>Montant</th><th className="pr-4 text-right">Actions</th></tr></thead><tbody className="divide-y">{charges.map(c => <tr key={c.id}><td className="p-4">{new Date(c.charge_date+'T00:00:00').toLocaleDateString('fr-FR')}</td><td className="font-semibold">{c.label}</td><td>{categoryLabel(c.category)}</td><td className="font-bold">{Number(c.amount).toLocaleString('fr-FR')} FCFA</td><td className="pr-4"><div className="flex justify-end gap-1"><button title="Voir" onClick={() => setView(c)} className="p-2"><Eye className="h-4 w-4" /></button><button title="Modifier" onClick={() => { setEdit(c); setModalOpen(true) }} className="p-2"><Pencil className="h-4 w-4" /></button><button title="Supprimer" onClick={() => setDeleteId(c.id)} className="p-2 text-red-600"><Trash2 className="h-4 w-4" /></button></div></td></tr>)}{!charges.length && <tr><td colSpan={5} className="p-12 text-center text-slate-400">{loading ? 'Chargement...' : 'Aucune charge enregistrée.'}</td></tr>}</tbody></table></div>
    </div>
